...
import cartIconBlack from '../../../assets/icons/cartIconBlack.png';
    ...
    <div className="flex justify-center h-[60px] shadow-sm sticky top-0 z-50 bg-white">
      <header className="flex flex-row items-center justify-between p-6 w-full ">
        ...
        <Link to="/cart" className="relative">
          <img src={cartIconBlack} alt="Cart" className="h-[24px]" />
          {count > 0 && (
            <span className="absolute -top-2 -right-2 bg-[#5BEF20] text-black text-xs px-1 rounded-full">
              {count}
            </span>
          )}
        </Link>
